...
// Description	: client initialisation file
	...
	, m_pClientSocket(0)
{
	...
	m_bOnline = true;
	m_bOnlineCopyToSend = true;
}

/***********************
* ~CClient
* @author: Sally Gillbanks
* @parameter: Destroys the client instance and free's it's memory
* @return: 
********************/
CClient::~CClient()
{
	m_bOnline = false;
	m_bOnlineCopyToSend = false;
...
bool CClient::Initialise()
{
	...
	m_strAddress = m_pClientSocket->GetLocalAddress() + ":" + std::to_string(_usClientPort);
...
********************/
std::vector<std::string> CClient::GetServerList()
{
	//Use a boolean flag to determine if a valid server has been chosen by the client or not
	bool _bServerChosen = false;

	float fOldTime = (float)glutGet(GLUT_ELAPSED_TIME);
	float fCurrentTime = (float)glutGet(GLUT_ELAPSED_TIME);
	do {
...
#pragma endregion _GETSERVER_
	...
	std::vector<std::string> vecStringServerList;
	for (unsigned int i{}; i < m_vecServerAddr.size(); ++i)
	{
		vecStringServerList.push_back(ToString(m_vecServerAddr[i]));
	}

	return(vecStringServerList);
}
/***********************
* SelectServer
* @author: Sally Gillbanks
...
* @return: bool
********************/
bool CClient::SelectServer(unsigned int _iServer)
{
	...
	if (_iServer > m_vecServerAddr.size())
	{
		return false;
	}
	m_ServerSocketAddress.sin_family = AF_INET;
	m_ServerSocketAddress.sin_port = m_vecServerAddr[_iServer].sin_port;
	m_ServerSocketAddress.sin_addr.S_un.S_addr = m_vecServerAddr[_iServer].sin_addr.S_un.S_addr;
	std::string _strServerAddress = ToString(m_vecServerAddr[_iServer]);
	m_bDoBroadcast = false;
	m_pClientSocket->DisableBroadcast();

	TPacket _packet;

	std::string strHandshake = _strServerAddress + " " + m_strAddress;
	_packet.Serialize(HANDSHAKE, JUSTATYPE, const_cast<char*>(strHandshake.c_str()));
	SendData(_packet.PacketData);
	m_strUsername = ToString(_cUserName);

	m_ClientReceiveThread = std::thread(&CClient::ReceiveData, this);
	return true;
}
...
bool CClient::BroadcastForServers()
{
	...
	TPacket _packet;
	_packet.Serialize(BROADCAST, JUSTATYPE, "Broadcast to Detect Server");
...
* @return: string
********************/
std::string CClient::GetUsername() const
{
...
void CClient::Reconnect()
{
	...
	////				while (_uiServerIndex > m_vecServerAddr.size() || 0 > _uiServerIndex)
	////				{
	////					std::cout << "You can not choose that number" << std::endl;
	////
	////					std::cout << "Choose a server number to connect to :";
	////					gets_s(_cServerChosen);
	////
	...
	//
	//	TPacket _packet;
	//	_packet.Serialize(HANDSHAKE, JUSTATYPE, _cUserName);
	//	SendData(_packet.PacketData);
	//	m_strUsername = ToString(_cUserName);
}